...
// Gestione completa interfaccia utente, display e interazioni
...
class UIManager {
    ...
    showCharacterCreation() {
        const backgrounds = [
            { id: 'pescatore', nome: 'Pescatore Esperto', bonus: 'Forza +2, Agilità +1' },
            { id: 'sognatore', nome: 'Sognatore Riflessivo', bonus: 'Empatia +2, Saggezza +1' },
            { id: 'esploratore', nome: 'Giovane Esploratore', bonus: 'Agilità +2, Astuzia +1' }
        ];
        
        const bgHTML = backgrounds.map(bg => `
            <button class="choice-button" onclick="ui.selectBackground('${bg.id}')">
                ${bg.nome}<br><small>${bg.bonus}</small>
            </button>
        `).join('');
        
        this.showModal('⚔️ Creazione Personaggio', `
            <p>Scegli il background di Trejano prima che il destino bussi alla sua porta:</p>
            ${bgHTML}
        `);
    }
    
    selectBackground(backgroundId) {
        const character = Character.createCustomCharacter({
            background: backgroundId,
            appellativo: this.getAppellativo(backgroundId)
        });
    ...
    getAppellativo(background) {
        const appellativi = {
            pescatore: 'il Pescatore',
            sognatore: 'il Sognatore',
            esploratore: 'l\'Esploratore'
        };
        return appellativi[background] || 'il Giovane';
    }
